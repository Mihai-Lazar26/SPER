...
solver.minimize(mtimes(casadi.transpose(c),x))
...
plt.plot(a[0:2],\
         [((mtimes(casadi.transpose(c),xsol)-c[0]*a[0])/c[1]).full().reshape(-1,),\
          ((mtimes(casadi.transpose(c),xsol)-c[0]*a[1])/c[1]).full().reshape(-1,)])
...
# define the cost to be minimized
solver.minimize(mtimes(mtimes(casadi.transpose(x),0.5*H),x)\
                +mtimes(casadi.transpose(f),x))
...
xunc=-mtimes(np.linalg.inv(np.linalg.cholesky(H)),f).full().reshape(-1,)
...
scaling=np.sqrt(mtimes(casadi.transpose(xsol-xunc),mtimes(0.5*H,xsol-xunc)))
L=np.linalg.cholesky(H)
theta=np.linspace(0,2*np.pi,100)

points=mtimes(np.linalg.inv(L),\
            casadi.transpose(np.concatenate((scaling*np.cos(theta),\
